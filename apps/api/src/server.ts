...
import { ScopusClient, ScopusClientError } from '@scopus/scopus-client';
...
export const buildServer = async (dependencies: ServerDependencies = {}): Promise<FastifyInstance> => {
  ...
  const client = dependencies.client ?? (config.apiKey ? new ScopusClient({
    apiKey: config.apiKey,
    institutionToken: config.institutionToken,
  ...
  server.get('/health', async () => ({ status: 'ok', service: 'scopus-api' }));
  ...
  server.get('/api/search', async (request, reply): Promise<SearchResponse | unknown> => {
    ...
    if (!client) {
      return reply.code(503).send(errorBody('SCOPUS_NOT_CONFIGURED', 'Set SCOPUS_API_KEY before using the search endpoint.'));
    }
    try {
      return await client.searchAndHydrate(parsed.data.q, parsed.data.limit, config.hydrationConcurrency);
    } catch (error) {
      if (error instanceof ScopusClientError) {
      ...
      return reply.code(502).send(errorBody('SCOPUS_REQUEST_FAILED', 'The Scopus request could not be completed.'));
    }
  });
